<?php 
	...
	function userlist($page, $total, $istotal){
		...
		$query = $database->execute("SELECT u.UserName,
										   u.LoginName, 
										   ut.Name UserType,
										   DATE_FORMAT(e.DateHired, '%m/%d/%Y') DateRegistered,
										   IFNULL(lh.Status, 'Active') `Status`,
										   IFNULL(DATE_FORMAT(loghist.LoginTime, '%m/%d/%Y %h:%i:%s %p'), 'N/A') LastLoginTime,
										   u.ID UserID
									FROM userloginhistory loghist
									INNER JOIN `user` u ON u.id = loghist.UserID
									...
									WHERE DATE(loghist.LoginTime) = DATE(NOW())
									AND loghist.LogoutTime IS NULL 
									ORDER BY IFNULL(DATE_FORMAT(loghist.LoginTime, '%m/%d/%Y %h:%i:%s %p'), 'N/A') DESC
				$limit");
