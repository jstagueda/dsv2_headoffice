 ...
 */
    ...
    if($post['action'] == 'generate'){
        ...
        try{
            $res = $mysqli->query("CALL spProcessServiceFee($SFlevel,$CampaingMonth,$CampaingYear)");
            ...
            if($res){
               $status = $res->fetch_object()->Status;
                ...
                if($status){
                    ...
                    $returns['message'] = 'No one passed on service fee.';
                }  
            }
        }catch(Exception $e){
            $returns['status'] = 'error';
    ...
    if($post['action'] == 'lists'){
        ...
        if($res->num_rows > 0){
            ...
            $returns['status'] = 'error';
            $returns['message'] = 'No one passed for service bonus.';
            $returns['lists'] = $lists;
        }
