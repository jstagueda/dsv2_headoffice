...
function confirmSave(){
	...
	if ($( "#chargeto" ).val()==0)
	{
        popinmessage("Please select if what department this PWS will be charged to.");
        $('[name=chargeto]').focus();
								...
								<tr>
									...
									<td>
										<select name="chargeto" class="txtfield" id="chargeto">
											<option value="0"><b>[Choose Here]</b></option>
											<?php 
												...
												while($codem = $codemaster->fetch_object()){
													echo "<option value='".$codem->Value."'>".$codem->CodeValue.' - '.$codem->Description."</option>";
												}
											?>
										</select>
									</td>
