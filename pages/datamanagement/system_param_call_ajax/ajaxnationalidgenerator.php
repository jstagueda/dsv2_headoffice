...
<?php
	...
	$parameterq = $database->execute("  SELECT settings.settingvalue  PATHFORNATID,
												   (
													  SELECT settings.settingvalue 
													  FROM settings 
												  WHERE settings.SettingCode = 'PATHFORBMLIST'       
												   ) PATHFORBMLIST,
												   (
													  SELECT settings.settingvalue 
													  FROM settings 
												  WHERE settings.SettingCode = 'PATHPROCESSEDNATID'      
												   ) PATHPROCESSEDNATID
											FROM settings 
											WHERE settings.SettingCode = 'PATHFORNATID'
										");
	if(!$parameterq->num_rows > 0 )
	{
		die();
	}
	else
	{
		while($p = $parameterq->fetch_object() )
		{
			$dir  =$p->PATHFORNATID;
	        $dir2 =$p->PATHPROCESSEDNATID;
	        $dir3 =$p->PATHFORBMLIST;
		}
	}
	
	#$dir  = "/var/www/html/sid/sid_files/disk1/db/ds.dt/NationalID";
	#$dir2 = "/var/www/html/sid/sid_files/disk1/db/ds.dt/BMList-P";
	#$dir3 = "/var/www/html/sid/sid_files/disk1/db/ds.dt/BMList";
	...
	$date   = date("mdy", strtotime($txtStartDate));
	
	$command = "ls ".$date.".* >/dev/null 2>&1 && cp ".$date."* ".$dir;

	exec(" cd ".$dir3." ; ".$command); //
		...
		while (($file = readdir($dh)) !== false)
		{
		  ...
		  else
		  {
			 ...
			 {			
				...
				#validate if with loaded bmlist greater than the said file
				$valid = 0;
				...
				if($valid == '1')
				{
					...
					$name             = str_replace("'",'',trim($fields[6]));
					$lastname         = str_replace("'",'',trim($fields[7]));
					$firstname        = str_replace("'",'',trim($fields[8]));
					$middlename       = str_replace("'",'',trim($fields[8]));
					...
					#look if HO Modified 
					$homod = 0; 
					$validateifHOModified = $database->execute("
																  Select * 
																  FROM tmpcustfornatid 
																  WHERE  branch = '$bcode' and dsv2code = '$custcode'
																  and tmpcustfornatid.Trantype in ('APPT','AFF','HOMODIFICATION') 
															   ");									   
					if($validateifHOModified->num_rows > 0 )
					{ 	
						if($rem2 == '' || $rem == '')
						{
							$homod = 1; 
						...
						while($cq = $custq->fetch_object() )
						{
							...
							if($homod == '0' )
							{
								...
								$database->execute("
												...
												SET s.payoutoroffset      = $PayoutOrOffset,
													s.vatable             = $Vatable,
			 ...
			 if($filebranch == '')
			 {
				$filebranch = substr($file,7,strlen($file)-13);
				$SID_Filesq = $database->execute(" SELECT * 
														from  SID_Files 
														where SID_Files.Branch    = '$filebranch'
														  and SID_Files.FileDate  = '$filedate'
