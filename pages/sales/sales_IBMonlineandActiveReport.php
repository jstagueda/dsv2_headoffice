...
<script src="js/jxIBMonlineandActiveReport.js" language="javascript" type="text/javascript"></script>
<?php
	//setExpires(0);
	include IN_PATH.DS."scIBMonlineandActiveReport.php";
?>
...
<tr>
	<td valign="top">
		...
		<tr>
  		...
  		<tr>
    		<td class="txtgreenbold13">IBM On-Line Sales and Active Report</td>
    		<td>&nbsp;</td>
  		</tr>
		</table>
		<br />
		...
		<table width="95%"  border="0" align="center" cellpadding="0" cellspacing="1">
  		<tr>
    		...
    		<table width="50%"  border="0" align="left" cellpadding="0" cellspacing="1" class="bordersolo">
				...
				<tr>
				  <td align="right"><strong>IBM :</strong></td>
				  <td height="20">&nbsp;</td>
				  <td align="left"><select name="cboIBMfrom" class="txtfield" >
				    <option value="0" selected="selected">IBM from</option>
				    <?php							
																						
							if ($rs_cusdetails->num_rows)
							{
								while ($row = $rs_cusdetails->fetch_object())
								{
						 			$sel = "";
				    ...
				    <select name="cboIBMto" class="txtfield" >
				      <option value="0" selected="selected">IBM to</option>
				      <?php							
																						
							if ($rs_cusdetails2->num_rows)
							{
								while ($row = $rs_cusdetails2->fetch_object())
								{
						 			$sel = "";
						 			echo "<option value='".$row->Code."' $sel >".$row->Code."</option>";	
								}
								$rs_cusdetails2->close();
							}
						?>
		          </select></td>
			    </tr>
    		  ...
    		  <td height="20" align="right"></td>
    		  <td height="20"></td>
    		  <td height="20" align="left"><input name="btnSubmit" type="button" class="btn" value="Submit" onclick="return showPage(0, <?php echo $branchid; ?>,  '<?PHP echo $campgnid; ?>', '<?PHP echo $ibmcode; ?>', '<?PHP echo $ibmfrom; ?>', '<?PHP echo $ibmto; ?>');" /></td>
  		  </tr>
    		<tr>
	...
	<script>
		...
		window.onload = showPage(0, <?php echo $branchid; ?>,  <?PHP echo $campgnid; ?>, '<?PHP echo $ibmcode; ?>', '<?PHP echo $ibmfrom; ?>', '<?PHP echo $ibmto; ?>');    
	</script>
