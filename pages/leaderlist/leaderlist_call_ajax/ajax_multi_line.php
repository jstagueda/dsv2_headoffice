...
function AddPagination($RPP, $num, $pageNum)
{
	...
	if($_POST['request'] == "fetch data"){
		...
		if($_POST['txtProductCode'] != ""){
		 ...
		 $offset = ($pageNum - 1) * $RPP;
		 $q = $database->execute("SELECT * FROM promo WHERE PromoTypeID = 2".$search." ".$whereID);
		 $num = $q->num_rows;
		 $pagination = AddPagination($RPP, $num, $pageNum);
		 $result['pagination'] = array('page'=>$pagination);
		 
		 $q = "select * from promo WHERE PromoTypeID = 2 ".$search." ".$whereID." order by ID desc limit ".$offset.", ".$RPP;
	...
	if($_POST['request'] == "search"){
			...
			$RPP		 = 8;
			$offset = ($pageNum - 1) * $RPP;
			$q = $database->execute("SELECT * FROM promo WHERE PromoTypeID = 2 and $Search");
			$pagination = AddPagination($RPP, $num, $pageNum);
			$result['pagination'] = array('page'=>$pagination);
		
		
			$dbr = $database->execute("SELECT * FROM promo WHERE PromoTypeID = 2 AND StatusID != 8 AND Code LIKE '%".$Search."%'
								   OR Description LIKE '%".$Search."%' AND PromoTypeID = 1 AND StatusID != 8 order by ID desc limit ".$offset.",".$RPP);
			if($dbr->num_rows){
			while($row = $dbr->fetch_object()){
				 $ID = $row->ID;
				 $Code = $row->Code;
