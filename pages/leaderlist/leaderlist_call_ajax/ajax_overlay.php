...
function AddPagination($RPP, $num, $pageNum)
{
	...
	if($_POST['request'] == "fetch data"){
		...
		if($_POST['txtProductCode'] != ""){
			...
			$PromoIncentivesID = $database->execute("SELECT b.ID FROM promobuyin a
													...
													LEFT JOIN product c ON a.ProductID = c.ID
													INNER JOIN promoentitlement pe ON pe.PromoBuyinID = a.ID
													INNER JOIN promoentitlementdetails ped ON ped.PromoEntitlementID = pe.ID
													INNER JOIN product p ON p.ID = ped.ProductID
													WHERE (c.Code = '".$txtProductCode."' OR p.Code = '".$txtProductCode."')
													AND b.StatusID <> 8
													AND b.PromoTypeID = 3
													GROUP BY b.ID");
		 ...
		 $offset = ($pageNum - 1) * $RPP;
		 $q = $database->execute("SELECT * FROM promo WHERE PromoTypeID = 3".$search." ".$whereID);
		 $num = $q->num_rows;
		 $pagination = AddPagination($RPP, $num, $pageNum);
		 $result['pagination'] = array('page'=>$pagination);
		 
		 $q = "select * from promo WHERE PromoTypeID = 3 ".$search." ".$whereID." order by ID desc limit ".$offset.", ".$RPP;
	...
	if($_POST['request'] == "search"){
			...
			$RPP		 = 8;
			$offset = ($pageNum - 1) * $RPP;
			$q = $database->execute("SELECT * FROM promo WHERE PromoTypeID = 3 and $Search");
			$pagination = AddPagination($RPP, $num, $pageNum);
			$result['pagination'] = array('page'=>$pagination);
		
		
			$dbr = $database->execute("SELECT * FROM promo WHERE PromoTypeID = 3 AND StatusID != 8 AND Code LIKE '%".$Search."%'
								   OR Description LIKE '%".$Search."%'  AND StatusID != 8 order by ID desc limit ".$offset.",".$RPP);
			if($dbr->num_rows){
			while($row = $dbr->fetch_object()){
				 $ID = $row->ID;
				 $Code = $row->Code;
