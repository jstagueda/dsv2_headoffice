...
function AddPagination($RPP, $num, $pageNum)
{
	...
	if($_POST['request'] == "fetch data"){
		...
		if($_POST['txtProductCode'] != ""){
			...
			$PromoIncentivesID = $database->execute("SELECT b.ID FROM loyaltypromobuyin a
													 INNER JOIN loyaltypromo b ON a.LoyaltyPromoID = b.ID
													 INNER JOIN product c ON a.ProductID = c.ID
													 WHERE c.Code = '".$txtProductCode."' and  GROUP BY b.ID");
		 ...
		 $offset = ($pageNum - 1) * $RPP;
		 $q = $database->execute("SELECT * FROM loyaltypromo WHERE Changed in (0,1) ".$search." ".$whereID);
		 $num = $q->num_rows;
		 //pagination
			$pagination = AddPagination($RPP, $num, $pageNum);
			$result['pagination'] = array('page'=>$pagination);
		 //end pagination
		 $q = "select * from loyaltypromo WHERE Changed in (0,1) ".$search." ".$whereID." order by ID desc limit ".$offset.", ".$RPP;
