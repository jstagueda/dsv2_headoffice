...
class tpiPromoAndPricing
{
	...
	public function spSelectSinglePromoDetById($database, $prmSID, $promoTypeID)
	{
					$query = $database->execute("SELECT * FROM (
																SELECT prm.ID prmID, pb.ProductID pbProdID, pb.MinQty pbMinQ, pb.MinAmt pbMaxQ, ped.Quantity pedQ, ped.EffectivePrice pedEF,
																...
																pmg.Code pmgcode, pmg.ID pmgid, pb.ID PromoBuyinID
																FROM promo prm
	...
	public function spSelectSinglePrmDetbyIDEnt($database, $prmSID)
	{
		$query = $database->execute("SELECT * FROM (select
									 ...
									 where prm.promotypeid = 1 and prm.ID = ".$prmSID.") a GROUP BY prodid2");
		return $query;
	}
	...
	public function spUpdatePromoEntitlementDetails($database, $promoEntitleID, $prodid1, $minimum1, $savings, $pmg1, $promoEntitleIDDetails, $savings, $criteria1, $produp1)
	{
