...
class StoredProcedures {
  ...
  public function spLLCheckDuplicateTempRollingForecast($database)
	...
	*/
	...
	public function spInsertTmpProdMaster($database,$prodCode, $prodName, $uom, $prodLine, $csp, $pmgCode, $pmStatus, $itemType, $brandold, $formold, $lastReceiptDate, $productType, $cost, $lastpodate, $prodCode_2nd, $prod_category, $stockingtype, $linetype, $mt, $brand, $subbrand, $form, $subform, $size)
	{
		$sp = "Call spInsertTmpProdMaster('$prodCode', '$prodName', '$uom', '$prodLine', $csp, '$pmgCode', '$pmStatus', '$itemType', '$brandold', '$formold', '$lastReceiptDate', '$productType', $cost, '$lastpodate','$prodCode_2nd', '$prod_category', '$stockingtype', '$linetype', '$mt', '$brand', '$subbrand', '$form', '$subform', '$size');"; 	
		$rs = $database->execute($sp);
		return $rs;
	}
...
public function spSelectListInvoiceProdExchange($database,$v_customerID, $v_search)
	...
	*/
	...
	public function spCreateLog($database, $txnid, $tablename, $fieldname, $moduleid, $oldvalue, $newvalue, $logtype, $remarks, $userid )
	{
		
		$sp = "INSERT INTO `logs`(`recordidno`,`tablename`,`fieldname`,`moduleid`,`oldvalue`,`newvalue`,`logtype`,`remarks`,`userid`,`enrollmentdate`)
									values($txnid,'$tablename','$fieldname',$moduleid,$oldvalue, $newvalue, '$logtype','$remarks',$userid, NOW())";
