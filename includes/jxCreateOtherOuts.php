<?php
		...
		if ($rs_product->num_rows)
		{
			...
			while ($row = $rs_product->fetch_object())
			{
				...
				echo "<tr class='$alt'>".
					 ...
					 "<td width='20%' height='20' class='borderBR'>$pcode".
					 "<td width='20%' height='20' class='borderBR'>$pname".
					 "<input name='hdnInventoryID[]' type='hidden' value='$invid'></td>".
					 "<td width='10%' align='center' class='borderBR'>&nbsp;</td>".
					 "<td width='10%' align='center' class='borderBR'>$soh</td>".
				...
				echo "</select>".
					 "</span></td>".
					 "<td width='10%' align='center' class='borderBR'>".
	...
	{	
		$rs_io = $sp->spSelectTmpInventoryOutDetails($database, $session->emp_id, $session->createid);
		
		echo "<table width='100%'  border='0' cellpadding='0' cellspacing='1' class='bgFFFFFF'>";
  
	  
		$rowcount = 0;
		while ($row = $rs_io->fetch_object())
		{
	...
	{ 
		...
		foreach ($inv as $key=>$IID)
		{
			if ($IID != "")
			{
				...
				$reason		 = $rsn[$key];
				$sort		 = 1;
				$affected_row = $sp->spInsertTmpInventoryOutDetails($database, $uid, $createid, $inventoryid, $uomid, $quantity, $reason, $sort);
			}
		}
		
		echo "Successfully added Inventory Out detail(s).";		
	}
	elseif (isset($_POST["action"]) && $_POST["action"] == "Remove")
	{
		...
		foreach ($inv as $key=>$IID)
		{
			$affected_row = $sp->spDeleteTmpInventoryOutDetails($database,$uid, $createid, $IID);	
		}
