...
$rsIBM=$tpi->selectCustomerIBM($custID);
if($rsIBM->num_rows) {

  while($getIBM=$rsIBM->fetch_object()) {
  
    if($getIBM->CustomerTypeID==99) $isEmployee=1;    
...
if($rsCustomerStatus->num_rows) {
  ...
  while($getStatus=$rsCustomerStatus->fetch_object()) $customerStatus=$getStatus->statusID;    
}

if($MTDNCFT=='') $MTDNCFT='0.00';
  ...
  if($rsGetGSUNetwork->num_rows) {
    ...
    while($GSUNetwork=$rsGetGSUNetwork->fetch_object()) {
      ...
      if($rsARBalance->num_rows) {
        ...
        while($ARBalance=$rsARBalance->fetch_object()) $outstandingbalancegsu=($ARBalance->GrossAmount-$ARBalance->TotalPayment);        
      }
      
      $rsPenalty=$sp->spSelectPenaltyByCustomerID($database, $gsucustomerID);
      if($rsPenalty->num_rows) {
      
        while($Penalty=$rsPenalty->fetch_object()) $outstandingpenaltygsu=$Penalty->OutstandingAmount;                
      }
      
      if($outstandingbalancegsu!=0 || $outstandingpenaltygsu!=0) $gsualert=1;           
    }
  }
}
