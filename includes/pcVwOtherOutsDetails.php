<?php 
	...
	{		
		 try
		 {
		  ...
		  $affected_rows = $sp->spDeleteInventoryOut($database, $txnid);
		  if (!$affected_rows)
			{
	...
	{		
		  try
		  {
			  ...
			  {  
				  ...
				  $affected_rows = $sp->spUpdateInventoryOutConfirm($database,$txnid, $remarks, $cby);
				  if (!$affected_rows)
						{
						throw new Exception("An error occurred, please contact your system administrator.");
						}
		 ...
		 try
		 {
		 	$database->beginTransaction();
			  ...
			  foreach ($_POST['chkID'] as $key=>$value) 
			  {
				  ...
				  $affected_rows = $sp->spDeleteInventoryOutDetails($database,$txnid, $prodid);
			 	 if (!$affected_rows)
					{
					throw new Exception("An error occurred, please contact your system administrator.");
					}
			  ...
			  redirect_to("../index.php?pageid=31.1&tid=$txnid&msg=$message");
		 }
		catch(Exception $e)
		{
			$database->rollbackTransaction();
