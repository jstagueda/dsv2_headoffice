<?php
	...
	if (isset($_POST['btnSave'])){
		try
		{
			...
			if($rs_code_exist->num_rows){
				...
				$rs_promoid = $sp->spInsertPromoHeaderOverlay($database, $promocode, $promodesc, $startdate, $enddate, 3, $breqid, $min_qty, $min_amt, $session->emp_id, $incn, $isplusplan);
				if (!$rs_promoid)
				{
				...
				//update here..
				$database->execute("update promo set IsForNewRecruit=".$IsForNewRecruit." where ID=".$promoID);
				//update end here..
	...
	//			}
				...
				redirect_to("index.php?pageid=178.1&inc=$incn");				
			}
		}
		catch(Exception $e)
		{
