<?php
	...
	if(isset($_POST['btnUpdate']))
	{
		...
		if($_POST['hCOA'] != '')
		{
			$IBMID = htmlentities(addslashes($_POST['hCOA']));
		}
		...
		if($_POST['txtbdaydealer'] != '')
		{
			...
			$cd_bday= date("Y-m-d", strtotime($_POST['txtbdaydealer']));	
		...
		else
		{
			$cd_approvedcl =  1;
                 ...
                 * @author: jdymosco
                 * @date: May 18, 2013
                 * @explanation: Removed the old value handler of recruiter field with the new one below because the old
                 *               way is not correct, it is passing the actual customer code and not it's id which the database
                 *               table value design for the field should be customer ID.
		...
		else
		{
			$cd_recruiter =  'null';
                ...
                if($_POST['txtRecruiteAcctNo'] != '' )
		{
                        $cd_recruiter=htmlentities(addslashes($_POST['txtRecruiteCustID']));	
		}
		else
		{
			$cd_recruiter =  'null';
		...
		if ($cd_dealertype != 1 || $cd_dealertype != 4)
		{
			if($_POST['cboBIRCOR'] != 0 )
			{
				$cd_bircorid= htmlentities(addslashes($_POST['cboBIRCOR']));	
			}
			else
			{
				$cd_bircorid = 1;
			...
			if($_POST['cboBIROR'] != 0 )
			{
				$cd_birorid= htmlentities(addslashes($_POST['cboBIROR']));	
			}
			else
			{
				$cd_birorid = 1;
			...
			if($_POST['cboVAT'] != 0 )
			{
				$cd_vatid= htmlentities(addslashes($_POST['cboVAT']));	
			}
			else
			{
				$cd_vatid = 1;
			...
			if($_POST['cboCommission'] != 0 )
			{
				$cd_comid= htmlentities(addslashes($_POST['cboCommission']));	
			}
			else
			{
				$cd_comid = 1;
		...
		if($id == "")
		{
			try
			{
				...
				$affected_rows = $sp->spInsertDealer($database, $cd_lname, $cd_fname, $cd_mname, $cd_bday, $cd_nickname, $cd_telno, $cd_mobileno, $cd_address, 
				$cd_zipcode, $cd_class, $cd_dealertype, $cd_gsutype, $cd_ibmno, $cd_ibmname, $cd_zone, $cd_lstay, $cd_marital, $cd_educational, $cd_TIN, $cd_igscode, $cd_areaid,
				$cd_directsellexp, $cd_empstatus, $cd_credittermid, $cd_character, $cd_capacity, $cd_capital,
				$cd_condition, $cd_calculatedcl, $cd_recommendedcl, $cd_approvedcl, $cd_remarks, $cd_pdaid,$IBMID,$cd_ibmcode,$cd_recruiter);			
				
				$txtbx = $_POST["textbx"];
				foreach ($txtbx as $key=>$ID) 
				{
					$insert = $sp->spInsertCompany($database, -1, $ID);
				}
				
				if ($cd_dealertype != 1 || $cd_dealertype != 4)
				{
					$result = $sp->spInsertUpdateBankInfo($database, -1, $cd_bircorid, $cd_birorid, $cd_vatid, $cd_comid);
				}
				...
				redirect_to("index.php?pageid=69&msg=$message&ibm=$cd_ibmno");
			...
			catch(Exception $e)
			{
				...
				$errmsg = $e->getMessage()."\n";	
				redirect_to("index.php?pageid=69&msg=$errmsg");
			}
		}
		...
		if($id > 0)
		{
			try
			{
				...
				$affected_rows = $sp->spUpdateDealer($database, $id, $cd_lname, $cd_fname, $cd_mname, $cd_nickname, $cd_telno, $cd_mobileno, $cd_address, 
				$cd_zipcode, $cd_class, $cd_dealertype, $cd_gsutype, $cd_ibmno, $cd_ibmname, $cd_zone, $cd_lstay, $cd_marital, $cd_educational,
				...
				if ($cd_dealertype != 1 || $cd_dealertype != 4)
				{
					$result = $sp->spInsertUpdateBankInfo($database, $id, $cd_bircorid, $cd_birorid, $cd_vatid, $cd_comid);
				}
				
				if (!$affected_rows && !$insert)
				{
					throw new Exception("An error occurred, please contact your system administrator.");
				}
