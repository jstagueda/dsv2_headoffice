...
$query = $sp->spSelectInventoryInSTACount($database, $vSearch);
...
if ($num > 0) 
{
	...
	elseif ($maxPage>=1 AND $type=='con') 
	{
		...
		//Get all the rows
		$query = $sp->spSelectInventoryInSTA($database, $offset, $RPP, $vSearch);
		$num = $query->num_rows;
		...
		while($row = $query->fetch_object()) 
		{
		  ...
		  echo "<tr align='center' class='$alt'>
					  ...
					  <td height='20' class='borderBR'><a href='index.php?pageid=30.1&tid=$id' class='txtnavgreenlink'>$row->DocumentNo.$row->ToBranchCode</a></td>
