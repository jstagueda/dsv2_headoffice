<?php
 	...
 	try 
	{
		...
		if ($statusid_inv == 21)
		{
			...
			redirect_to("../index.php?pageid=51.1&msg=$message&TxnID=$prid");				
		}
		else
		{
			$affected_rows = $sp->spUpdateProvisionalReceiptCancel($database, $prid, $reasonId, $remarks, $cancelledBy);
			if (!$affected_rows)
			{
		...
		{
			$database->beginTransaction();*/
			$updatestatus = $sp->spUpdateLockStatus($database, 'provisionalreceipt', 0, 0, $prid);
			if (!$updatestatus)
			{
				throw new Exception("An error occurred, please contact your system administrator.");
			}
