...
function getconsolidatednetworklist($database, $HOGeneralID)
{
    $query = $database->execute(" SELECT b.code bcode, c.HOGeneralID , c.Code ccode , c.Name cnamne, IF(c.StatusID = 5 ,'TERMINATED','ACTIVE') statusid,
										 ...
										 IFNULL((SELECT DATE(sfm.Appointment_Date) FROM sfm_appthist sa WHERE sa.customerid = c.customerid AND sa.sfm_level = 2 ORDER BY ID LIMIT 1 ),'') bmcdate, 
										 ...
										 IF(c.CustomerTypeID =2,'',
										 (IFNULL((SELECT DATE(sfm.Appointment_Date) FROM sfm_appthist sa WHERE sa.customerid = c.customerid AND sa.sfm_level = (IF(c.CustomerTypeID = 4, 4, 3)) ORDER BY ID LIMIT 1 ),'')) )  bmfdate
...
function getconsolidatednetwork($database, $ibmfrom, $ibmto, $status, $istotal, $page, $total, $branch)
{
	...
	$query = $database->execute(" Select b.code bcode, c.HOGeneralID , c.Code ccode , c.Name cnamne, IF(c.StatusID = 5 ,'TERMINATED','ACTIVE') statusid,
								   ...
								   (SELECT DATE(sfm_appthist.appointment_date) 
									...
									if(c.CustomerTypeID =2,'',((SELECT DATE(sfm_appthist.appointment_date) 
								     FROM sfm_appthist 
									 WHERE sfm_appthist.CustomerID = c.customerid
