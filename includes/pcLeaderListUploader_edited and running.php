 ...
 */
	...
	{	
		...
		if ($uType > 4)
		{
			...
			//file_get_contents spInsertTmpSingleLinePromo
			...
			if (is_uploaded_file($_FILES['file']['tmp_name']))
			{
				...
				try
				{
					...
					foreach ($rows as $row)				 	{
						...
						else if ($uType == 6) //multiline promo - buyin
						{
							...
							$Uploader_rows = $sp->spInsertTmpMultiLineBuyinPromo($database, 
                                                                    $mb_promocode,    mysql_real_escape_string($mb_promodesc), 
                                                                    $mb_promosdate,   $mb_promoedate, 
                                                                    $mb_buyintype,    $mb_buyinminqty, 
                                                                    $mb_buyinminamt,  $mb_buyinplevel, 
                                                                    $mb_buyinpcode,   mysql_real_escape_string($mb_buyinpdesc), 
						...
						else if ($uType == 9) //overlay promo - entitlement
						{
							...
							$Uploader_rows = $sp->spInsertTmpOverlayEntitlementPromo($database, 
                                                                $oe_promocode, 
					...
					if ($uType == 5)
					{
						...
						$details = $sp->spSelectTmp_PromoSingleLine($database); // select
						if ($details->num_rows)
						{
										...
										else
										{
											...
											else
											{
												...
												if($rs_promoid->num_rows)
												{
													...
													//update promo header
													$sp->spUpdatePromoHeaderByID($database, 
                                                                                                                $promoID, 
                                                                                                                mysql_real_escape_string($promo_desc), 
                                                                                                                $promo_sdate, 
                                                                                                                $promo_edate, 
                                                                                                                $promo_isplusplan);
												}
											...
											if ($promoID != 0)
											{
												...
												if (!$promo_buyin_exist->num_rows)
												{
													if ($buyin_type == 1)
													{
														$rs_promobuyin_child = $sp->spInsertPromoBuyIn($database, 
                                                                                                                        $promoID, 'null', 
                                                                                                                        $buyin_type, 
                                                                                                                        $buyin_minqty, 'null', 'null', 'null', 
                                                                                                                        $buyin_plevel, 
                                                                                                                        $buyin_prodid, 'null', 
                                                                                                                        $promo_sdate, 
                                                                                                                        $promo_edate, 1);
												...
												else
												{
													...
													if ($buyin_type == 1)
													{
														$sp->spUpdatePromoBuyInByID($database, 
                                                                                                                        $buyinparentID, 
                                                                                                                        $promoID, 
                                                                                                                        $buyin_type, 
                                                                                                                        $buyin_minqty, 'null', 'null', 'null', 
						...
						if ($multiline->num_rows)
						{
							while ($row = $multiline->fetch_object())
							{
								...
								if ($buyin_prod->num_rows)
								{
									...
									{
										//insert promo header
										$rs_promoid = $sp->spInsertPromoHeader($database, 
                                                                                        $promo_code, 
                                                                                        mysql_real_escape_string($promo_desc), 
                                                                                        $promo_sdate, 
                                                                                        $promo_edate, 2, 
										...
										$rs_promobuyin_parent = $sp->spInsertPromoBuyIn($database, 
                                                                                        $promoID, 'null', 1, 'null', 'null', 'null', 'null', 3, 'null', 
                                                                                        $buyin_reqtype, $promo_sdate, $promo_edate, 0);
						...
						if ($overlay->num_rows)
						{
							while ($row = $overlay->fetch_object())
							{
								...
								if ($buyin_prod->num_rows)
								{
									...
									{
										//insert promo header
										$rs_promoid = $sp->spInsertPromoHeaderOverlay($database, $promo_code, 
                                                                                        mysql_real_escape_string($promo_desc), 
                                                                                        $promo_sdate, 
                                                                                        $promo_edate, 3, 
                                                                                        $buyin_type, 
									...
									else
									{
										...
										else
										{
											...
											if($rs_promoid->num_rows)
											{
												...
												//update promo header
												$sp->spUpdatePromoHeaderByID($database, $promoID, 
                                                                                                        mysql_real_escape_string($promo_desc), $promo_sdate, $promo_edate, 2);
											}
											else
											{
												//promo code already exist in diff promo type
		...
		else
		{
			...
			if (is_uploaded_file($_FILES['file']['tmp_name']))
			{
					...
					if (($handle = fopen($_FILES['file']['tmp_name'], "r")) !== FALSE) {
					    ...
					    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
					        ...
					        for ($c=0; $c < $num; $c++) {
					        	if ($uType == 1) {
						        	switch($c){
						        		...
						        		case 1:
						        			//$prodLineDescription = str_replace("'", "''", $data[$c]);
                                                                                $prodLineDescription = mysql_real_escape_string(str_replace("'", "''", $data[$c]));
						        			break;
						        		case 2:
						        			$campaign = $data[$c];
						        			break;
						        	...
						        	switch($c){
						        		...
						        		case 3:
						        			$prodLineDescription = mysql_real_escape_string(str_replace("'", "''", $data[$c]));
                                                                           // '".mysql_real_escape_string($_POST[PromoDescription])."',
						        			break;
						        		case 4:
						        			// ignore column
						        			break;
					        ...
					        try {
					        	...
					        	if ($uType == 1) {
					        		$sp->spLLInsertTempAOP($database,$prodLineCode,
                                                                        mysql_real_escape_string($prodLineDescription),$campaign,$gmargin,$gprofit,$aopnet,$aopunits);
					        	}
					        	else if ($uType == 2) {
					        		$sp->spLLInsertTempRollingForecast($database,$prodLineCode,
                                                                        mysql_real_escape_string($prodLineDescription),$campaignID,$promoCode,$rfunits,$type);
					        	}
					          else if ($uType == 3) {
					        		...
					        		$sp->spLLInsertTempDummyCost($database,$prodLineCode,
                                                                        mysql_real_escape_string($prodLineDescription),$dummyCost,$startDate,$endDate);
					        	}
					        }
					        catch (Exception $e){
				...
				catch (Exception $e) 
				{
					...
					$errmsg = "Invalid Interface file.";//grace
					//redirect_to("../index.php?pageid=135&errmsg=$errmsg");
				}
			}			
		}
