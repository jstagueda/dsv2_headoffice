...
$query = $sp->spSelectSOCount($database, $vSearch, $fromDate, $toDate);
...
*/
		echo "<table width='100%'  border='0' cellpadding='0' cellspacing='1' class='tab'>
                    <tr align='center'>                     
                      <td width='15%' class='bdiv_r'><b>SO Number</b></td>
					   <td width='15%' class='bdiv_r'><b>Document Number</b></td>
					  <td width='20%' class='bdiv_r'><b>Customer Name</b></td>
					  <td width='15%' class='bdiv_r' height='20'><b>SO Date</b></td>                      					  
                      <td width='15%' ><b>Status</b></td>
                    </tr>";
		//Get all the rows
		$query = $sp->spSelectSalesOrder($database,$offset, $RPP, $vSearch, $fromDate, $toDate);				
		$num=$query->num_rows;
		...
		while($row = $query->fetch_object()) {
				...
				$sono = $row->SONo;
				$sodocno = $row->DocumentNo;				
				$id = $row->SOID;				
				$txndate = $row->TxnDate;
				$customer = $row->Customer;			
				$status = $row->TxnStatus;
			if($row->TxnStatusID == 9)
			{
			
				echo "<tr align='center' class='$alt'>
						  <td  height='20' class='borderBR'>
						  <a href='index.php?pageid=35.1&TxnID=$id' class='txtnavgreenlink'>$sono</a></td>
						  <td  class='borderBR'> $sodocno </td>					  
						  <td  class='borderBR'> $customer </td>
						  <td  class='borderBR'>$txndate </td>					  
						  <td  class='borderBR'>$status </td>
					  </tr>";
			}
			if($row->TxnStatusID == 7)
			{
				echo "<tr align='center' class='$alt'>
						  <td  height='20' class='borderBR'>
						  <a href='index.php?pageid=35.2&TxnID=$id&msg=' class='txtnavgreenlink'>$sono</a></td>
						  <td  class='borderBR'> $sodocno </td>					  
						  <td  class='borderBR'> $customer </td>
						  <td  class='borderBR'>$txndate </td>					  
						  <td  class='borderBR'>$status </td>
			...
			if($row->TxnStatusID == 6)
			{
				echo "<tr align='center' class='$alt'>
						  <td  height='20' class='borderBR'>
						  <a href='index.php?pageid=35.3&TxnID=$id' class='txtnavgreenlink'>$sono</a></td>
						  <td  class='borderBR'> $sodocno </td>					  
						  <td  class='borderBR'> $customer </td>
						  <td  class='borderBR'>$txndate </td>					  
						  <td  class='borderBR'>$status </td>
