<?php
   ...
   global $database; 
	...
	if  (isset($_GET["mod"]) && $_GET["mod"] == "w"){
		...
		$rs_transfer = $sp->spSelectTmpTransferDetails($database, $session->emp_id, $session->createid);
		if($rs_transfer->num_rows)
		{
		  $dis = "disabled";
		  while($row = $rs_transfer->fetch_object())
		  {
		...
		echo "<input type='hidden' name='hdncboWarehouse' value='$warehouseid' />";
		echo "<select name='cboWarehouse' style='width:140px' class='txtfield' $dis>";
  	...
  	elseif  (isset($_GET["wid"]))
  	{
		...
		$rs_deswarehouse = $sp->spSelectDesWarehouse($wid);
		
		echo "<select name=\"cboDesWarehouse\" style=\"width:160px\" class=\"txtfield\">";
  	...
  	elseif  (isset($_GET["mod"]) && $_GET["mod"] == "t")
	{
		...
		if ($rs_product->num_rows)
		{
			...
			while ($row = $rs_product->fetch_object())
			{
				...
				echo "<tr class='$alt'>".
					 "<td width='5%' height='20' align='center' class='borderBR'>$cnt".
	...
	{
		$rs_ad = $sp->spSelectTmpTransferDetails($database, $session->emp_id, $session->createid);
		echo "<table width='100%'  border='0' cellpadding='0' cellspacing='1' class='bgFFFFFF'>";
	...
	elseif (isset($_POST["action"]) && $_POST["action"] == "Add")
	{
		...
		foreach ($inv as $key=>$IID)
		{
			if ($IID != "")
			{
				...
				$affected_row = $sp->spInsertTmpTransferDetails($database, $uid, $createid, $inventoryid, $uomid, $quantity, $reason, $sort);
			}
		}
	}
	elseif (isset($_POST["action"]) && $_POST["action"] == "Remove")
	{
		...
		foreach ($inv as $key=>$IID)
		{
			$affected_row = $sp->spDeleteTmpTransferDetails($database, $uid, $createid, $IID);	
		}
