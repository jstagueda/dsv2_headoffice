...
function viewregularprice($limit,$istotal,$lastupd)
{
   global $database; 
   ...
   if($lastupd)
   {
	   $printcnd = "where pp.lastuploaddate=(SELECT LastUploadDate FROM productpricing ORDER BY lastuploaddate DESC LIMIT 1)";
   }
   else
   {
	   $printcnd = "";
		...
		$regprice = $database->execute(" 
											SELECT p.code,p.Description,pp.UnitPrice 
    ...
    if($num > 0) {
        ...
        for($page; $page <=  $upper; $page++) {
            ...
            if($page == $pageNum){
                ...
                $nav .= " <a style='cursor:pointer;' onclick=\"return displayall(".$page.")\">$page</a> ";
            }
        }
        ...
        if($pageNum > 1){
            $page  = $pageNum - 1;
            $prev  = "<img border='0' src='$PrevIc'  onclick=\"return displayall(".$page.")\" style='cursor:pointer;'> ";
            $first = "<img border='0' src='$FirstIc' onclick=\"return displayall(1)\"  style='cursor:pointer;'> ";
        }else{
        ...
        if($pageNum < $maxPage AND $upper <= $maxPage) {
            $page = $pageNum + 1;
            $next = " <img border='0' src='$NextIc' style='cursor:pointer;' onclick=\"return displayall(".$page.")\" >";
            $last = " <img border='0' src='$LastIc' style='cursor:pointer;' onclick=\"return displayall(".$maxPage.")\" >";
        }else{
