...
$query = $sp->spSelectPromoCount($database, 0, $RPP, 1, $scodedesc, $sprodcode, 0);
...
if ($num > 0) 
{
	...
	for($page; $page <=  $upper; $page++) 
	{
		...
		else
		{
			...
			$nav .= " <a style:'cursor:pointer' onclick='showPage(\"".$page."\", \"".$sprodcode."\")'>$page</a> ";
		}
	}
	...
	{
		$page  = $pageNum - 1;
		$prev  = "<img border='0' src='$PrevIc' onclick='showPage(\"".$page."\", \"".$sprodcode."\")' style:'cursor:pointer'> ";
		$first = "<img border='0' src='$FirstIc' onclick='showPage(\"1\", \"".$sprodcode."\")'  style:'cursor:pointer'> ";
	}
	else
	{
	...
	{
		$page = $pageNum + 1;
		$next = " <img border='0' src='$NextIc' onclick='showPage(\"".$page."\", \"".$sprodcode."\")'  >";
		$last = " <img border='0' src='$LastIc'  onclick='showPage(\"".$maxPage."\", \"".$sprodcode."\")' >";
	} 
	else 
	{
	...
	elseif ($maxPage>=1 AND $type=='con') 
	{
		echo "<table width='100%'  border='0' cellpadding='0' cellspacing='1'>
				<tr align='center' class='txtdarkgreenbold10 tab'>
					<td width='10%' height='20' class='txtpallete bdiv_r'><div align='left' class='padl5'>Promo Code</div></td>
					<td width='35%' height='20' class='txtpallete bdiv_r'><div align='left' class='padl5'>Promo Description</div></td>			
					...
					<td width='10%' height='20' class='txtpallete padr5'><div align='center'>Date Created</div></td>
				</tr>";
                    
		//Get all the rows
		//$query = $sp->spSelectPromo2($database, $offset, $RPP, 1, $scodedesc, $sprodcode, 0);
		//$query  =  $database->execute('select * from `promo` where StatusID in (6,7) and PromoTypeID = 1
									   //order by StartDate,Code limit  '.$offset.', '.$RPP.'');
		//$query = $sp->spSelectPromoCount($database, $offset, $RPP, 1, $scodedesc, $sprodcode, 0);
		$query = $database->execute('SELECT DISTINCT prm.ID, prm.Code, prm.Description, 
									 prm.`StartDate`, prm.`EndDate`, prm.`StartDate` DateCreated, prm.EnrollmentDate
									 FROM `promo` prm
									 WHERE prm.StatusID IN (6,7) AND prm.PromoTypeID = 1 AND (prm.`Code` LIKE UPPER("%'.$scodedesc.'%") OR prm.`Description` LIKE UPPER("%'.$scodedesc.'%"))
									 ORDER BY prm.StartDate,prm.Code LIMIT  '.$offset.', '.$RPP);
		$num = $query->num_rows;
		...
		while($row = $query->fetch_object()) 
		{
			...
			$tmpenrolldate = strtotime($row->EnrollmentDate);
			$enrolldate = date("m/d/Y", $tmpenrolldate);
		  ...
		  echo "<tr align='center' class='$alt'>
					  ...
					  <td class='borderBR'><div align='center'>$enrolldate</div></td>
				<!--	  <td height='20' class='borderBR'><div align='left' class='padl5'><a href='javascript:void(0)' onclick='return openPopUp($prmSID);' class='txtnavgreenlink'>$startdate</a></div></td> -->
				  </tr>";
	  	}
		//Close table
		echo "</table>";
	...
	echo "<table width='100%'  border='0' cellpadding='0' cellspacing='1' class='txtdarkgreenbold10 tab'>
		<tr align='center'>
			<td width='10%' height='20' class='txtpallete bdiv_r'><div align='left' class='padl5'>Promo Code</div></td>
					...
					<td width='10%' height='20' class='txtpallete bdiv_r'><div align='center'>Start Date</div></td>
					<td width='10%' height='20' class='txtpallete bdiv_r'><div align='center'>End Date</div></td>
					<td width='10%' height='20'><div align='center' class='txtpallete padr5'>Date Created</div></td>
		</tr>
