<?php
	...
	else if(isset($_POST['btnSave'])) 
	{
			...
			else
			{
				...
				if($custquery->num_rows)
				{
					...
					while($res = $custquery->fetch_object())
					{
						...
						if(($StatusID == '5' && $customertypeid != 4) || $AccountType != 'BMC-E' )
						{
							...
							createtmpcustfornatid($mid,$Gender,$withOR2,$tin,$Name,$firstname,$lastname,$middlename,$Birthdate,$appdate,$bcode,$TelNo,$MobileNo,$StreetAdd,$AreaID,
							                      $PayoutOrOffset2,$Vatable2,$ApplicableBonusCodes,$bank_acct_name,$bank_acct_num,$bank_name,$affbranchcode,$HOGeneralID,$dsv1_igscode,$code,$manager_code,'BMC-E','BMC-E');				  
							#echo 'f4';	
							
							#echo 'f5';	
						    $message = "Transaction was successfully process<br>"."BM: ".$code.'-'.$Name.'<br>'.'Endorsement Branch:'.$affbranchcode.'<br>Endorsement Effectivity Date:'.$appdate ;	
						}
						else
						{
							$message = 'Branch Code: '.$affbranchcode.'<br>'.'Already exist.'; 
							redirect_to("index.php?pageid=180.8&msg=$message&branch=$BID&tid=$id&HOGeneralID=$HOGeneralID");	
						}
					}
				}
				else
				{
					...
					$message = "Transaction was successfully process."."BM: ".$code.'-'.$Name.'\n'.'Mother Branch:'.$affbranchcode.'\n Effectivity Date:'.$appdate ;
					#echo $CustomerID.'nnnn';	
				}	
			}
			$database->commitTransaction();
					#redirect_to("index.php?pageid=180.8&msg=$message&tid=$id&HOGeneralID=$HOGeneralID");
		...
		catch(Exception $e) 
		{
			...
			$errmsg = $e->getMessage()."\n";
			redirect_to("index.php?pageid=180.8&tid=$id&msg=$errmsg&userid=$userid");	
		}
	}
