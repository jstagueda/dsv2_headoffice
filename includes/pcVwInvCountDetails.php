<?php 
	...
	{	
		...
		{	
		  ...
		  $affected_rows = $sp->spDeleteInventoryCount($database, $txnid);
		  if (!$affected_rows)
			{
	...
	{		
		try
		{
		  ...
		  $rs_detailsall = $sp->spSelectInvCountDetailsByID($database, $txnid);
		  ...
		  {  
			  ...
			  $affected_rows = $sp->spUpdateInventoryCountConfirm($database,$txnid, $remarks, $cby);
			  if (!$affected_rows)
					{
					throw new Exception("An error occurred, please contact your system administrator.");
					}
	...
	{		
		 try
		 {
		  ...
		  foreach ($_POST['chkID'] as $key=>$value) 
		  {
			  ...
			  $affected_rows = $sp->spDeleteInventoryCountDetails($database, $txnid, $prodid);
			  if (!$affected_rows)
				{
				throw new Exception("An error occurred, please contact your system administrator.");
				}
		  ...
		  redirect_to("../index.php?pageid=32.1&tid=$txnid&msg=$message");	
		 }
		catch(Exception $e)
		{
			$database->rollbackTransaction();
