...
$query = $sp->spSelectPromoCount($database, 0, $RPP, 2, $scodedesc, $sprodcode, 0);
...
if ($num > 0) 
{
	...
	elseif ($maxPage>=1 AND $type=='con') 
	{
		...
		//Get all the rows
		$query = $sp->spSelectPromo2($database, $offset, $RPP, 2, $scodedesc, $sprodcode, 0);
		$num = $query->num_rows;
		...
		while($row = $query->fetch_object()) 
		{
			...
			$tmpcdate = strtotime($row->EnrollmentDate);
			$createdate = date("m/d/Y", $tmpcdate);
            $buyinid = $row->ID;
            $prode = $row->Code; 
		  echo '<tr align="center" class="$alt">
		             <input name="hdnbuyinID" type="hidden" id="hProdID1" value="' . $row->ID . '"/>
					  <td height="20" class="borderBR"><div align="left" class="padl5"><a href="pages/promo_pricing/promo_popup.php" onclick="return popup(' . $buyinid . ',' . "'$prode'" . ');" class="txtnavgreenlink">' .$row->Code .'</a></div></td>
					  ...
					  <td class="borderBR"><div align="center">'.$createdate.'</div></td>
				  </tr>';
	  	}
		//Close table
		echo "</table>";
