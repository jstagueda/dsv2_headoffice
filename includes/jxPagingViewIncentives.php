...
$query = $tpiIncentives->spSelectIncentiveCount($database, $RPP,  $scodedesc, $sprodcode);
...
if ($num > 0) 
{
	...
	if ($maxPage>=1 AND $type=='nav'){
		...
		//Get all the rows
		$query = $tpiIncentives->spSelectIncentives($database, $RPP,  $scodedesc, $sprodcode);
		$num = $query->num_rows;
		...
		while($row = $query->fetch_object()) {
		  ...
		  echo "<tr align='center' class='$alt'>
                      ...
                      <div align='left' class='padl5'><a href='javascript:void(0)' onclick='return openPopUp($prmSID);' class='txtnavgreenlink'>$row->Code</a></div></td>
