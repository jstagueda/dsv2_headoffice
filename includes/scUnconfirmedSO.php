...
if($rs_header->num_rows) {
  ...
  while($row=$rs_header->fetch_object()) {
    ...
    $isEmployee=0;
    if($row->CustomerTypeID==99) $isEmployee=1;                    
  }
}
  ...
  if($statusid_inv==21) {
    ...
    if($valid==1) {
      ...
      try {
        ...
        $insertSI=$sp->spInsertSIBackOrder($database, $v_CustomerID, $txnid, $v_DocumentNo, $v_txndate, $v_remarks, $v_GrossAmount, $v_TotalCPI, $v_BasicDiscount, $v_TotalCFT, $v_TotalNCFT, $v_AddtlDiscount, $v_VatAmount, $v_AddtlDiscountPrev, $v_NetAmount);
