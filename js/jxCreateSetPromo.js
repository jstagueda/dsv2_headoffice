...
function getSelectionProductCriteriaList(text, li) 
{
	...
	tmp = li.id;
	tmp_val = tmp.split("_");
	h = eval('document.frmCreateSet.hProdID_criteria');
	i = eval('document.frmCreateSet.txtCriteria');
	j = eval('document.frmCreateSet.txtCDescription');
...
function getSelectionProductList2(text, li) 
{
	...
	tmp = li.id;
	tmp_val = tmp.split("_");
	h = eval('document.frmCreateSet.hEProdID' + ctr);
	i = eval('document.frmCreateSet.txtEProdCode' + ctr);
	j = eval('document.frmCreateSet.txtEProdDesc' + ctr);
	k = eval('document.frmCreateSet.txtEQty' + ctr);
	l = eval('document.frmCreateSet.hEUnitPrice' + ctr);
//	m = eval('document.frmCreateSet.hEpmgid' + ctr);
	n = eval('document.frmCreateSet.cboEPMG' + ctr);
	o = eval('document.frmCreateSet.cboECriteria' + ctr);
...
function addRow2() 
{	
	var ml = document.frmCreateSet;
	var table = document.getElementById('dynamicEntTable');
	var rowCount = table.rows.length;
	var cnt2 = document.frmCreateSet.hEntIndex.value;
	
	if(cnt2 >  rowCount)
   	{
   		
   		 var a = eval('document.frmCreateSet.txtEProdDesc' + cnt2);
   	...
   	{
   		 var a = eval('document.frmCreateSet.txtEProdDesc' + rowCount);
...
//     }
	...
	element1.setAttribute("style", "width: 80%");
	...
	element2.setAttribute("style", "width: 92%");
	...
	element3.setAttribute("style", "width: 85%");
	...
	element4.setAttribute("style", "width: 50%; text-align: right");
...
//	element5.appendChild(option2);	
	...
	element5.setAttribute("style", "width: 80%");
	cell6.appendChild(element5)
	document.frmCreateSet.hEntIndex.value = index;
...
function confirmSave()
{
	...
	if (eCount == 1)
	{
		var a = eval('document.frmCreateSet.txtEProdCode1');
...
function deleteRow2(i){
    ...
    { 
    	...
    	document.frmCreateSet.hEntitlementCnt.value = rowCount;
    }
    else
    {
        return false;
...
function selection()
{  
	  var type = document.frmCreateSet.cboType.value;

	  if(type == 1)
	  {
		  document.frmCreateSet.txtTypeQty.readOnly = true;
		  document.frmCreateSet.txtTypeQty.value = "";
	  }
	  else
	  {
		  document.frmCreateSet.txtTypeQty.readOnly = false;
	  }
} 

function CheckInclude()
{	
	var ci = document.frmCreateSet.elements["chkSelect[]"];

	for(i=0; i< ci.length; i++)
	{
		if(ci[i].checked == false)
		{
			document.frmCreateSet.chkAll.checked = false;
		}
	}
			
	if (document.frmCreateSet.elements["chkSelect[]"].value > 1)
	{
		if(ci.checked == false)
		{
			document.frmCreateSet.chkAll.checked = false;
...
{
	var elms = document.frmCreateSet.elements;
...
function disableEnterKey(a, e, cnt)
{
     ...
     else if (str.substring(0, sub2) == field2 && key == 13)
     {
         ...
         var code = eval('document.frmCreateSet.txtEProdCode' + eval(ctr + 1));
         code.focus();
         code.select();
         return false;
     }
