...
{ 
	...
	*/
	...
	ibmfrom = document.frmValuationReport.cboIBMfrom.length;
	ibmto = document.frmValuationReport.cboIBMto.length;	
	...
	for (z = 0; z < ibmfrom; z++ ) {
		if (document.frmValuationReport.cboIBMfrom[z].selected) {
			vibmfrom = document.frmValuationReport.cboIBMfrom[z].value
		}
	}	

	for (y = 0; y < ibmfrom; y++ ) {
		if (document.frmValuationReport.cboIBMto[y].selected) {
			vibmto = document.frmValuationReport.cboIBMto[y].value
		}
	}
	...
	// HTTP 1
	var url="includes/jxIBMCollectionDue.php"
	url=url+"?p="+str
	url=url+"&t=con"
	url=url+"&b="+branchid
	url=url+"&ibf"+vibmfrom
	url=url+"&ibt"+vibmto	
	url=url+"&fd="+fromdate
	url=url+"&td="+todate
