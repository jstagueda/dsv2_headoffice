  ...
  txnID = eval('document.frmCreateSalesOrder.hTxnID');
	...
	$j.ajax({
                   ...
                   success: function(test){
					   ...
					   $j('#prodlist').load('includes/jxCalculatePriceSO.php','mode=get&refresh=' + refreshProductlist + '&custID=' + custID.value + '&backOrder=' + backOrder.value,function(){ 
							 
							 //alert(hasIncentive.value);
	...
	$j.ajax({
                   ...
                   success: function(outCustomerDetails){
						...
						if(tmp_val[12] == 1)
						{
							...
							location.href='index.php?pageid=34';
							return false;
						...
						if(tmpPenalty.toFixed(2) != 0.00)
						{
							alert("System can't create Sales Order for the selected dealer due to the following balances:\n\n" +					
									"\t\t Total Past Due Amount: P "+ AvalableCredit.value + "\n"+
