...
var xmlHttp2
var xmlHttp3
function showPage(str, frmdte, todte, rid) { 
	//Function that gets called
	//Currently we only call one other sub, but this could change

	showStates(str, frmdte, todte, rid)
}

function showStates(str, frmdte, todte, rid) { 
	...
	var url="includes/jxPagingDMCMRegister.php"
	url=url+"?p="+str
	url=url+"&t=nav"
	url=url+"&fromdate="+frmdte
	url=url+"&todate="+todte
	url=url+"&rrid="+rid
...
function openPopUp(obj1,obj2,obj3) 
{
	...
	var objWin;
		...
		popuppage = "pages/sales/sales_memoreg_print.php?frmdte="+obj1+"&todte="+obj2+"&rid="+obj3;
