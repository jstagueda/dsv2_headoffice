...
{ 
	...
	blen = document.frmBCEProfileReport.cboBranch.length
	clen = document.frmBCEProfileReport.cboCampaignRange.length
	
	for (x = 0; x < blen; x++) {
		if(document.frmBCEProfileReport.cboBranch[x].selected) {
			branchid = document.frmBCEProfileReport.cboBranch[x].value
		}
	}
	
	for (x = 0; x < clen; x++) {
		if(document.frmBCEProfileReport.cboCampaignRange[x].selected) {
			campgnid = document.frmBCEProfileReport.cboCampaignRange[x].value
		}
	}
	
	// HTTP 1
	var url="includes/jxBCEProfileReport.php"
	url=url+"?p="+str
	url=url+"&t=con"
	url=url+"&b="+branchid
	url=url+"&c="+campgnid
