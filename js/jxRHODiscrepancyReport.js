...
function showPage(pgNum, date, edate, bcode, mtype) 
{ 			    
	//Function that gets called
	//Currently we only call one other sub, but this could change

	showStates(pgNum, date, edate, bcode, mtype); 
}

function showStates(pgNum, date, edate, bcode, mtype) 
{ 
	...
	var url="includes/jxPagingRHODiscrepancyReport.php"
	url=url+"?pgNum="+pgNum
	url=url+"&t=nav"
	url=url+"&date=" + date
	url=url+"&edate=" + edate
...
function openPopUp(obj1, obj2, obj3) 
{
	var objWin;
	popuppage = "pages/inventory/inv_rhodiscrepancy_print.php?date=" + obj1 + "&bcode=" + obj2 + "&mtype=" + obj3;
...
$(function(){
    $('[name=cboBCodex]').autocomplete({
        source  :   function(request, response){
            $.ajax({
                type    :   "post",
                dataType:   "json",
                data    :   {searched   :   request.term},
                url     :   "includes/jxPagingRHODiscrepancyReport.php",
        ...
        select  :   function(event, ui){
            $('[name=cboBCode]').val(ui.item.ID);
        }
    });
    
    $('[name=txtDate], [name=txtDate1]').datepicker({
        changeMonth :   true,
        changeYear  :   true
    });
});