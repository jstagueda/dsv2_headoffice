...
{
	var elms = document.frmInventoryOutDetails.elements;
...
{	
	msg = '';
	obj = document.frmInventoryOutDetails.elements;
	objiid = document.frmInventoryOutDetails.elements["chkIID[]"];	
	...
	{
		// TEXT BOXES
		if (obj["cboInventoryOutType"].selectedIndex == 0) msg += '   * Inventory Out Type \n';
...
{ 
	...
	var url="includes/jxCreateOtherOuts.php"
	url=url+"?mod=t"		
	url=url+"&wrid="+wid
	url=url+"&srch="+srch
...
{ 
	document.getElementById("dvInventoryOutDetails").innerHTML="<img border='0' src='images/ajax-loader.gif'>"
	...
	{ 
		//Update the Div tag with the outputted text
		document.getElementById("dvInventoryOutDetails").innerHTML=xmlHttpProd.responseText 
	} 
}
...
function getInventoryOut(action, thisDiv) 
{
	 mydiv = thisDiv  
	 if (action == "Add")
	 {
		 ...
		 else
		 {
			 poststr = "action="   + encodeURI(action) + 
			  ...
			  makePOSTRequest('includes/jxCreateOtherOuts.php', poststr)
