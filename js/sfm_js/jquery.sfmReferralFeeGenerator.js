...
$(function($){
    ...
    $('#SFM-RFForm').on('submit', function(e){
        var self = $(this), ppost = {};
        var SFLevel = self.find('select[name="RFSFL"]').val();
        var CampaignCode = self.find('input[name="RFCM"]').val();
        ...
        if(SFLevel == ''){
            dialogMessage('Select level first to process referral fee.');
            return false;
        }
        ...
        dialogMessage('Processing referral fee, please wait...');
        $.ajax({
            cache: false,
            url: 'pages/sfm/param_ajax_calls/referralfee_processor.php',
            type: 'POST',
            data: ppost,
    ...
    function frmListsLevelsSelect(response){
        ...
        if(res.levels.length > 0){
            for(var x = 0; x < res.levels.length; x++){
                html+= '<option class="RFSFL" value="'+ res.levels[x].codeID +'">'+ res.levels[x].codeID + ' - ' + res.levels[x].description +'</option>';
            }

            $('.RFSFL').remove();
            $('#SFM-RFForm select[name="RFSFL"]').append(html);
        }
    }
