/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 *
 * @author: jdymosco
 * @date: March 19, 2013
 */
$(function($){
    documentWriteSFMParamCustoms();
    getLevelsForSelect();

    $('#SFM-CBForm').on('submit', function(e){
        var self = $(this), ppost = {};
        var SFLevel = self.find('select[name="CBSFL"]').val();
        var CampaignCode = self.find('input[name="CBCM"]').val();
        var action = self.find('input[name="action"]').val();

        ppost = {'SFLevel':SFLevel,'CampaignCode':CampaignCode,'action' : action};
        loaderTxt = '<b>Processing, please wait...</b>';

        if(SFLevel == ''){
            dialogMessage('Select level first to process candidacy bonus.');
            return false;
        }

        if(CampaignCode == ''){
            dialogMessage('Select campaign code to process candidacy bonus.');
            return false;
        }

        dialogMessage('Processing candidacy bonus, please wait...');
        $.ajax({
            cache: false,
            url: 'pages/sfm/param_ajax_calls/candidacy_bonus_processor.php',
            type: 'POST',
            data: ppost,
            success: processFormSubmitResponse
        });

        e.preventDefault();
        return false;
    });

    function processFormSubmitResponse(response){
        var res = $.parseJSON(response);

        if(res.status == 'success'){
            dialogMessage('Preparing lists of results, be patient...');
            $.ajax({
                cache: false,
                url: 'pages/sfm/param_ajax_calls/candidacy_bonus_processor.php',
                type: 'POST',
                data: {'action':'lists'},
                success: processResponseLists
            });
        }else{
            dialogMessage(res.message);
        }
    }

    function processResponseLists(response){
        var res = $.parseJSON(response);
        var html = '';

        if(res.lists.length > 0){
            for(var x = 0; x < res.lists.length; x++){
                html+= '<tr class="tbl-td-rows">';
                html+= '<td class="tbl-td-center td-bottom-border">'+ res.lists[x].Code +'</a></td>';
                html+= '<td class="tbl-td-center td-bottom-border">'+ res.lists[x].Name +'</td>';
                html+= '<td class="tbl-td-right td-bottom-border">'+ res.lists[x].EarnedBonusAmount +'</td>';
                html+= '<td class="tbl-td-right td-bottom-border">'+ res.lists[x].VAT +'</td>';
                html+= '<td class="tbl-td-right td-bottom-border">'+ res.lists[x].WithHoldingTax +'</td>';
                html+= '</tr>';
            }

            $('.tbl-td-rows').remove();
            $('.tbl-listing-div table').append(html);
        }else{
            $('.tbl-td-rows td').text(res.message);
        }

        $('#dialog-message').dialog('close');
    }

    function getLevelsForSelect(){
        $.ajax({
            url: 'pages/sfm/param_ajax_calls/level_ajax.php',
            type:'POST',
            data: { 'action':'get_levels' },
            success: frmListsLevelsSelect
        });
    }

    function frmListsLevelsSelect(response){
        var res = $.parseJSON(response);
        var html = '';

        if(res.levels.length > 0){
            for(var x = 0; x < res.levels.length; x++){
                html+= '<option class="CBSFL" value="'+ res.levels[x].codeID +'">'+ res.levels[x].codeID + ' - ' + res.levels[x].description +'</option>';
            }

            $('.CBSFL').remove();
            $('#SFM-CBForm select[name="CBSFL"]').append(html);
        }
    }

});


