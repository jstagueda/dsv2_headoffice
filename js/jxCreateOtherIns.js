...
{
	var elms = document.frmInventoryInDetails.elements;
...
{	
	msg = '';
	obj = document.frmInventoryInDetails.elements;
	objiid = document.frmInventoryInDetails.elements["chkIID[]"];	
	...
	{
		// TEXT BOXES
		if (obj["cboInventoryInType"].selectedIndex == 0) msg += '   * Inventory In Type \n';
...
{ 
	...
	var url="includes/jxCreateOtherIns.php"
	url=url+"?mod=t"		
	url=url+"&wrid="+wid
	url=url+"&srch="+srch
...
{ 
	document.getElementById("dvInventoryInDetails").innerHTML="<img border='0' src='images/ajax-loader.gif'>"
	...
	{ 
		//Update the Div tag with the outputted text
		document.getElementById("dvInventoryInDetails").innerHTML=xmlHttpProd.responseText 
	} 
}
...
function getInventoryIn(action, thisDiv) 
{
	 mydiv = thisDiv  
	 if (action == "Add")
	 {		 	
		 ...
		 else
		 {
			...
			makePOSTRequest('includes/jxCreateOtherIns.php', poststr)
