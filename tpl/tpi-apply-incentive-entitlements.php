...
# $tpi->insertAppliedPromoApplicationFunction($sales_order_session_id, $incentive_buyin_id, base64_encode(serialize('apply_incentive_entitlements('.$incentive_buyin_id.', \''.$_GET['applicable_incentive_entitlement_details_quantities'].'\');')));
...
$rsApplicableIncentiveEntitlements=$tpi->getApplicableIncentiveEntitlement($incentive_buyin_id, $entitlement_type, 0);
if($rsApplicableIncentiveEntitlements->num_rows) {
  
  while($field=$rsApplicableIncentiveEntitlements->fetch_object()) {        
    ...
    $entitlement_quantity=$entitlement_criteria==1?((int)$field->Quantity):((int)$applicable_incentive_entitlement_details_quantities[$entitlement_number++]);
    ...
    $applied_incentive_entitlements_html.='
    <tr>
...
echo $applied_incentive_entitlements_html;
