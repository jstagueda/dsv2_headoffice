...
$tpi->insertAppliedPromoApplicationFunction($sales_order_session_id, $promo_id, base64_encode(serialize('apply_multi_line_promo_entitlements('.$promo_id.', \''.$_GET['applicable_multi_line_promo_entitlement_details_quantities'].'\');')));
...
if($rsApplicableMultiLinePromoEntitlements->num_rows) {
  ...
  while($field=$rsApplicableMultiLinePromoEntitlements->fetch_object()) {        
    ...
    $applied_multi_line_promo_entitlements_html.='
    <tr>
...
echo $applied_multi_line_promo_entitlements_html;
