...
$tpi->insertAppliedPromoApplicationFunction($sales_order_session_id, $promo_id, base64_encode(serialize('apply_overlay_promo_entitlements_as_discount('.$promo_id.', \''.$_GET['applicable_overlay_promo_entitlement_details_quantities'].'\');')));
...
$rsApplicableOverlayPromoEntitlements=$tpi->getApplicableOverlayPromoEntitlement($database, $promo_id, $entitlement_type, 0);
if($rsApplicableOverlayPromoEntitlements->num_rows) {

  $applied_promo_entitlements_javascript='';
  
  while($field=$rsApplicableOverlayPromoEntitlements->fetch_object()) {        
    ...
    $entitlement_quantity=$entitlement_criteria==1?((int)$field->Quantity):((int)$applicable_overlay_promo_entitlement_details_quantities[$entitlement_number++]);
    ...
    $applied_promo_entitlements_javascript.='apply_promo_entitlement_as_discount(\''.$promo_code.'\', \''.$field->PromoID.'_3\', '.$field->ItemCode.', '.$entitlement_quantity.', '.$field->EffectivePrice.'); ';    
  }
}
