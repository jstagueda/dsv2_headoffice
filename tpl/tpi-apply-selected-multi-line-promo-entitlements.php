...
$tpi->insertAppliedPromoApplicationFunction($sales_order_session_id, $promo_id, base64_encode(serialize('apply_selected_multi_line_promo_entitlements('.$promo_id.', \''.$_GET['applicable_multi_line_promo_entitlement_details_ids'].'\', \''.$_GET['applicable_multi_line_promo_entitlement_details_quantities'].'\');')));
...
foreach($applicable_multi_line_promo_entitlement_details_ids as $key=>$applicable_multi_line_promo_entitlement_details_id) {
  ...
  if($rsApplicableMultiLinePromoEntitlement->num_rows) {            
    ...
    while($field=$rsApplicableMultiLinePromoEntitlement->fetch_object()) {            
      ...
      $applied_selected_multi_line_promo_entitlements_html.='
      <tr>
        <td align="center" class="borderBR padl5">
          <input type="hidden" id="applicable_multi_line_promo_entitlement'.++$x.'" name="applicable_multi_line_promo_entitlements[]" value="'.$field->PromoID.'_'.$field->EntitlementType.'_'.$field->PromoEntitlementDetailsID.'_'.$field->ProductID.'_'.$entitlement_quantity.'_'.$entitlement_outstanding_quantity.'_'.$field->EffectivePrice.'_'.$field->Savings.'_'.$field->PMGID.'" />'.$field->ItemCode.'
        </td>              
...
echo $applied_selected_multi_line_promo_entitlements_html;
